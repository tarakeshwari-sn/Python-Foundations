...
class KPIResponse(BaseModel):
    ...
    most_affected_country: str | None
    most_common_activity: str | None
    peak_decade: int | None

#App setup and calculation functions
...
#ETL Pipeline functions and routes
...
def update_attack(attack_id:int,data:SharkAttackUpdate,db:Session=Depends(get_db),current_user: str = Depends(get_current_user)):
...
def get_kpis(db: Session = Depends(get_db),current_user: str = Depends(get_current_user)):
    ...
    records = db.query(SharkAttack).all()
    if not records:
        raise HTTPException(status_code=404, detail="No data available")

    df=pd.DataFrame([{"fatal_clean": r.fatal_clean,"country": r.country,
            "activity": r.activity,"decade": r.decade} for r in records])
    ...
    return {"total_attacks": total_attacks,
...
def export_data_on_shutdown():
    ...
    try:
        records=db.query(SharkAttack).all()
        if not records:
            logger.info("No data to export on shutdown.")
            return

        df=pd.DataFrame([
            {"number": r.number,"year": r.year,
                "country": r.country,"activity": r.activity,
                "sex": r.sex,"age": r.age,
                "fatal_clean": r.fatal_clean,"decade": r.decade,"century": r.century
            }for r in records])
        export_to_json(df, JSON_OUTPUT)
        logger.info("Data exported successfully on shutdown.")
    finally:
        db.close()
